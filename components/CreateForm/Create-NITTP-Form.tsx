...
const CreateNITTPForm = () => {
  const [isPending, startTransition] = useTransition();

  const form = useForm<z.infer<typeof CreateNITTPSchema>>({
  ...
  const onSubmit = async (values: z.infer<typeof CreateNITTPSchema>) => {
    startTransition(async () => {
      const result = await CreateNITTPAction(values);
      if (result.error) {
        toast.error(result.error);
      } else {
        toast.success("NITTP 課程創建成功");
        form.reset();
      }
    });
  };
  ...
  return (
    <div className="bg-gray-800 text-white min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <h1 className="text-2xl font-bold mb-6">創建 NITTP 課程</h1>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-white">公司名稱</FormLabel>
                    <FormControl>
                      <Input
                        {...field}
                        disabled={isPending}
...
export default CreateNITTPForm;